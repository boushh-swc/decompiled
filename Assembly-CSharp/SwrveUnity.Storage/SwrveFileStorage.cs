...
namespace SwrveUnity.Storage
{
	public class SwrveFileStorage : ISwrveStorage
	{
		...
		public virtual void Save(string tag, string data, string userId = null)
		{
			...
			try
			{
				string fileName = this.GetFileName(tag, userId);
		...
		public virtual void SaveSecure(string tag, string data, string userId = null)
		{
			...
			try
			{
				...
				string data2 = SwrveHelper.CreateHMACMD5(data, this.uniqueKey);
			...
			try
			{
				...
				if (CrossPlatformFile.Exists(fileName))
				{
					...
					if (!string.IsNullOrEmpty(text))
					{
						...
						{
							string value = SwrveHelper.CreateHMACMD5(text, this.uniqueKey);
		...
		{
			string str = (this.swrvePath.Length <= 0) ? string.Empty : "/";
			return this.swrvePath + str + tag + ((userId != null) ? userId : string.Empty);
		}
	}
}
